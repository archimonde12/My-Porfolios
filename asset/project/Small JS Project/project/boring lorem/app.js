...
function GenerateLoremText(){
    result.innerHTML=``;
    let textClone=[...text];
    let numberOfLine=input.value;
    let random=0;
    for(let i=0;i<numberOfLine;i++){
        random=Math.floor(Math.random()*textClone.length)
        result.innerHTML+= `<p class="result">${textClone[random]}</p>`
        textClone.splice(random,1)
        if(textClone.length==0){textClone=[...text]}
    }
}
